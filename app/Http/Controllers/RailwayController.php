...
class RailwayController extends Controller
{
    ...
    public function find_train(Request $request)
    {
        ...
        Session::put('way',$way);
        $data = DB::select("select * from ( select trains.price,trains.train_id,trains.name as train_name,s1.name as depart_station ,s2.name as arrive_station ,trains.depart_time,trains.arrive_time from trains,station as s1,station as s2 where (s1.station_id=trains.depart_stationid and trains.arrive_stationid=s2.station_id )) as A where depart_station=? and arrive_station=? ",[$from,$to]);
        if($way=="two")
            foreach($data as $d)
            {
                $d->price=$d->price*2;
    ...
    function adminLogin(Request $req) {
        ...
        if($userData->email == $req->email && $userData->password == $req->password) {
            $req->session()->put('user_id', $userData->user_id);
